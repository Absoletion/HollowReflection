...
function showTown() {
  ...
  function openMarket() {
    ...
    const items = Object.values(Engine.MARKET_ITEMS).map(item => {
      ...
      const affordable = META.gold >= item.price && (!item.effect.dust || META.glassDust >= item.effect.dust);
      return `<div class="quest-preview"><div><b>${esc(item.name)}</b><span>${esc(item.description)}</span><span>${item.price.toLocaleString()} Gold · ${remaining}/${item.limit} remaining</span></div><button data-market-buy="${item.id}" ${!unlocked || !remaining || !affordable ? 'disabled' : ''}>${unlocked ? (remaining ? 'Buy' : 'Sold Out') : 'Locked'}</button></div>`;
    }).join('');
