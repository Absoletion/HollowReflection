...
for (let account = 0; account < 10000; account++) {
  const result = GameState.performSummon(Engine.normalizeSaveState({ sigils: 90, missionClears: Object.fromEntries([
    ...Array.from({ length: 10 }, (_, i) => [`act1_${i + 1}`, true]),
    ...Array.from({ length: 8 }, (_, i) => [`act2_${i + 1}`, true]),
    ['act3_1', true], ['act3_2', true], ['act3_3', true],
  ]) }), 'standard', 10, rng, { transactionId: `summon:sim:${account}` });
