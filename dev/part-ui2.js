...
function startBattle(key, partyKeys, onEnd) {
  ...
  function partyCard(u) {
    ...
    return `<div class="ucard e-${u.awakened ? 'hollow' : u.elem} ${canSelect ? 'selectable' : ''} ${selUid === u.uid ? 'sel' : ''} ${u.acted && u.alive ? 'acted' : ''} ${u.alive ? '' : 'dead'} ${isTarget ? 'targetable' : ''}" data-uid="${u.uid}" data-side="party">
      <div class="medal">${u.name[0]}</div>
      <div class="uname">${u.name}${u.awakened ? ' ◈' : ''}</div>
      <div class="urole">${Engine.ELEM_ICON[u.elem]} ${u.role}</div>
  ...
  function enemyCard(en, idx, dupes) {
    ...
    return `<div class="ucard enemy e-${en.elem} ${en.alive ? '' : 'dead'} ${isTarget ? 'targetable' : ''}" data-uid="${en.uid}" data-side="enemy">
      ...
      <div class="medal">${en.name[0]}</div>
      <div class="uname">${name}</div>
      <div class="urole">${Engine.ELEM_ICON[en.elem]} ${en.key === 'glasswright' ? '???' : 'hollowed'}</div>
  ...
  function render() {
    ...
    app.innerHTML = `
      <div class="battle">
        <div class="bhead">
          ...
          <span class="note">${NOTES[key] || ''} &nbsp; ${sigilHTML()}</span>
        </div>
        <div class="field">
    ...
    const lp = document.getElementById('logpane'); lp.scrollTop = lp.scrollHeight;
    wire();
  }
    ...
    setTimeout(() => {
      ...
      ov.innerHTML = `<div class="ov-card"><h2 style="color:var(--warn)">The line breaks.</h2>
        ...
        <div class="hubrow" style="justify-content:center;">
          ...
          <button id="ovtitle">Return to Title</button>
        </div></div>`;
      document.body.appendChild(ov);
      ov.querySelector('#ovretry').onclick = () => { ov.remove(); startBattle(key, partyKeys, onEnd); };
      ov.querySelector('#ovtitle').onclick = () => { ov.remove(); showTitle(); };
    }, 650);
  }
